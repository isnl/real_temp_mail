...
export async function handleEmailProcessing(message, env) {
    try {
        ...
        // 查找对应的临时邮箱记录
        const tempEmailRecord = await env.DB.prepare('SELECT * FROM temp_emails WHERE email = ? AND active = 1').bind(toEmail).first();
        if (!tempEmailRecord) {
    ...
    `).bind(tempEmailRecord.id, fromAddress, subject, textContent, htmlContent, verificationCode).run();
      ...
      VALUES (?, ?, ?, datetime('now', '+8 hours'))
    `).bind(tempEmailRecord.user_id, 'RECEIVE_EMAIL', `Received email from ${fromAddress} to ${toEmail}`).run();
    }
    catch (error) {
